...
namespace SexyFramework.Drivers.Graphics
{
	public class BaseXNARenderDevice : RenderDevice3D
	{
		...
		public override void DrawSprite(Image theImage, Color theColor, int theDrawMode, SexyTransform2D theTransform, Rect theSrcRect, bool center)
		{
			...
			Rectangle rectangle;
			rectangle..ctor(image.mAtlasStartX + theSrcRect.mX, image.mAtlasStartY + theSrcRect.mY, theSrcRect.mWidth, theSrcRect.mHeight);
			Texture2D texture2D = xnatextureData.mTextures[0].mTexture;
			Vector2 vector;
			vector..ctor(theTransform.mMatrix.M41, theTransform.mMatrix.M42);
			Vector2 vector2;
			if (center)
			{
				vector2..ctor((float)rectangle.Width / 2f, (float)rectangle.Height / 2f);
			}
			else
			{
				vector2..ctor(0f, 0f);
