...
namespace SexyFramework.Misc
{
	public static class SexyLocale
	{
		...
		public static string UCommaSeparate(uint theValue)
		{
			...
			int num = 64;
			int num2 = 0;
			if (text.get_Chars(num2) != SexyLocale.CHAR_MAX && text.get_Chars(num2) > '\0')
			{
				char c = SexyLocale.gThousandSep.get_Chars(0);
				int num3 = 0;
				while (theValue != 0U)
				{
					array[--num] = (char)(48U + theValue % 10U);
					theValue /= 10U;
					if (theValue != 0U && ++num3 == (int)text.get_Chars(num2))
					{
						array[--num] = c;
						num3 = 0;
						if (text.get_Chars(num2 + 1) > '\0')
						{
							num2++;
						}
					}
