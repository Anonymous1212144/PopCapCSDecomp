...
namespace SexyFramework.Graphics
{
	public class ImageFont : Font
	{
		...
		public virtual void DrawStringEx(Graphics g, int theX, int theY, string theString, Color theColor, Rect theClipRect, LinkedList<Rect> theDrawnAreas, ref int theWidth)
		{
			...
			{
				char mappedChar = this.GetMappedChar(theString.get_Chars(i));
				char c = '\0';
				if (i < theString.Length - 1)
				{
					c = this.GetMappedChar(theString.get_Chars(i + 1));
				}
				int num3 = num;
		...
		{
			char result;
			if (this.mFontData.mCharMap.TryGetValue(theChar, ref result))
			{
				return result;
			}
			return theChar;
		...
		public override int CharWidthKern(char theChar, char thePrevChar)
		{
			...
			if (this.mCachedCharPair.TryGetValue(theChar, ref dictionary))
			{
				if (dictionary.TryGetValue(thePrevChar, ref num))
				{
					return num;
				}
			}
