			...
			for (int i = 0; i < this.mParticleTypeInfo.size<ParticleTypeInfo>(); i++)
			{
				...
				if (!first.mSingle || first.mNumCreated <= 0)
				{
					...
					if ((float)(frame - particleTypeInfo.second) >= num2 || this.mUseAlternateCalcMethod)
					{
						...
						{
							float num4 = ((float)particleSettings.mNumber + Common.SAFE_RAND((float)particleVariance.mNumberVar)) * this.mLastScale.mNumberScale * this.mCurrentLifetimeSettings.mNumberMult / 6.66666651f;
