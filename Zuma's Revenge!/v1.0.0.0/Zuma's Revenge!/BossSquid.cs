	...
	public class BossSquid : BossShoot
	{
				...
				for (int j = 0; j < list.Count; j++)
				{
					...
					if (inkParticle.mImage == imageByID && inkParticle.mPostHitCount < Common._M(5))
					{
						for (int k = 0; k < 3; k++)
						{
							...
							float num3 = inkParticle.mX + array2[k] * (float)Math.Cos((double)(inkParticle.mAngle + 1.57079637f + array3[k]));
							float num4 = inkParticle.mY - array2[k] * (float)Math.Sin((double)(inkParticle.mAngle + 1.57079637f + array3[k]));
							num3 += (float)inkParticle.mPostHitCount * (float)Math.Cos((double)(inkParticle.mAngle - 1.57079637f)) * inkParticle.mInitSpeed;
							num4 -= (float)inkParticle.mPostHitCount * (float)Math.Sin((double)(inkParticle.mAngle - 1.57079637f)) * inkParticle.mInitSpeed;
		...
		protected override bool PreBulletUpdate(BossBullet b, int index)
		{
			...
			else if (Math.Abs(b.mState) == 2)
			{
				...
				inkParticle.mAngle = ((b.mShotType == 0) ? (-3.14159274f / Common._M(2f)) : num);
				if (b.mVX < 0f)
				{
					inkParticle.mAngle += 3.14159274f * Common._M(0.5f);
				}
				else
				{
					inkParticle.mAngle += 3.14159274f * Common._M(0.5f);
