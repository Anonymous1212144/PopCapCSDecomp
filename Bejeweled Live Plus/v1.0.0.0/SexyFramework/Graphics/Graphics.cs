	...
	public class Graphics : GraphicsState
	{
		...
		protected void CalcFinalColor()
		{
			if (this.mPushedColorVector.Count > 0)
			{
				...
				this.mFinalColor = new Color((int)Math.Min(255f, (float)(color.mRed * this.mColor.mRed) / 255f), (int)Math.Min(255f, (float)(color.mGreen * this.mColor.mGreen) / 255f), (int)Math.Min(255f, (float)(color.mBlue * this.mColor.mBlue) / 255f), (int)Math.Min(255f, (float)(color.mAlpha * this.mColor.mAlpha) / 255f));
				return;
			}
		...
		{
			this.SetDrawMode((int)theDrawMode);
		}
		...
		public void DrawTrianglesTexStrip(Image theTexture, SexyVertex2D[] theVertices, int theNumTriangles, Color theColor, int theDrawMode, float tx, float ty, bool blend)
		{
			...
			while (i < theNumTriangles)
			{
				...
				for (int j = 0; j < num; j++)
				{
					Graphics.DTTS_aList[j, 0] = theVertices[i];
					Graphics.DTTS_aList[j, 1] = theVertices[i + 1];
					Graphics.DTTS_aList[j, 2] = theVertices[i + 2];
					i++;
				}
				this.mRenderDevice.BltTriangles(theTexture, Graphics.DTTS_aList, num, theColor, theDrawMode, tx, ty, blend);
			}
		}
		...
		public int WriteString(string theString, int theX, int theY, int theWidth, int theJustification, bool drawString, int theOffset, int theLength, int theOldColor)
		{
			...
			{
				if (theString.get_Chars(i) == '^' && this.mWriteColoredString)
				{
					if (i + 1 < theLength && theString.get_Chars(i + 1) == '^')
					{
					...
					else
					{
						...
						else
						{
							for (int j = 0; j < 6; j++)
							{
								char c = theString.get_Chars(i + j + 1);
								int num3 = 0;
								if (c >= '0' && c <= '9')
								{
									num3 = (int)(c - '0');
				...
				{
					this.mStringBuilder.Append(theString.get_Chars(i));
				}
			}
		...
		public int WriteWordWrapped(Rect theRect, string theLine, int theLineSpacing, int theJustification, ref int theMaxWidth, int theMaxChars, ref int theLastWidth, ref int theLineCount, bool drawString)
		{
			...
			int num = color.ToInt();
			if (((long)num & (long)((ulong)(-16777216))) == (long)((ulong)(-16777216)))
			{
				num &= 16777215;
			}
			if (theMaxChars < 0)
			...
			while (i < theLine.Length)
			{
				...
				if (num8 > theRect.mWidth)
				{
					...
					if (num4 != -1)
					{
						...
						if (c != '\n')
						{
							while (i < theLine.Length && theLine.get_Chars(i) == ' ')
							{
								i++;
							}
						}
		...
		{
			this.SetAsCurrentContext();
			if (this.mRenderDevice is RenderDevice3D)
			{
				((RenderDevice3D)this.mRenderDevice).Flush();
			}
		}
