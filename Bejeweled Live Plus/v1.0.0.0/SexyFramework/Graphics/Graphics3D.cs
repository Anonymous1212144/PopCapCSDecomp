	...
	public class Graphics3D
	{
		...
		public void SetMasking(Graphics3D.EMaskMode inMaskMode, int inAlphaRef, float inFrontDepth, float inBackDepth)
		{
			...
			case Graphics3D.EMaskMode.MASKMODE_NONE:
				this.mRenderDevice.SetAlphaTest(Graphics3D.ECompareFunc.COMPARE_ALWAYS, 0);
				this.mRenderDevice.SetDepthState(Graphics3D.ECompareFunc.COMPARE_ALWAYS, false);
				this.mRenderDevice.SetBltDepth(inBackDepth);
				this.mRenderDevice.SetBlend(Graphics3D.EBlendMode.BLEND_DEFAULT, Graphics3D.EBlendMode.BLEND_DEFAULT);
				return;
			case Graphics3D.EMaskMode.MASKMODE_WRITE_MASKONLY:
			case Graphics3D.EMaskMode.MASKMODE_WRITE_MASKANDCOLOR:
				this.mRenderDevice.SetAlphaTest(Graphics3D.ECompareFunc.COMPARE_GREATER, inAlphaRef);
				this.mRenderDevice.SetDepthState(Graphics3D.ECompareFunc.COMPARE_ALWAYS, true);
				this.mRenderDevice.SetBltDepth(inFrontDepth);
				if (inMaskMode == Graphics3D.EMaskMode.MASKMODE_WRITE_MASKONLY)
				{
					this.mRenderDevice.SetBlend(Graphics3D.EBlendMode.BLEND_ZERO, Graphics3D.EBlendMode.BLEND_ONE);
					return;
				}
				this.mRenderDevice.SetBlend(Graphics3D.EBlendMode.BLEND_DEFAULT, Graphics3D.EBlendMode.BLEND_DEFAULT);
				return;
			...
			case Graphics3D.EMaskMode.MASKMODE_TEST_OUTSIDE:
				this.mRenderDevice.SetAlphaTest(Graphics3D.ECompareFunc.COMPARE_GREATER, 0);
				this.mRenderDevice.SetDepthState((inMaskMode == Graphics3D.EMaskMode.MASKMODE_TEST_OUTSIDE) ? Graphics3D.ECompareFunc.COMPARE_LESSEQUAL : Graphics3D.ECompareFunc.COMPARE_GREATEREQUAL, false);
				this.mRenderDevice.SetBltDepth(inBackDepth);
				this.mRenderDevice.SetBlend(Graphics3D.EBlendMode.BLEND_DEFAULT, Graphics3D.EBlendMode.BLEND_DEFAULT);
				return;
			default:
