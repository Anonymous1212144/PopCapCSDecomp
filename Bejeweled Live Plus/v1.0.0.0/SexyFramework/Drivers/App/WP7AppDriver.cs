...
namespace SexyFramework.Drivers.App
{
	public class WP7AppDriver : IAppDriver
	{
		...
		{
			this.mApp = appBase;
			this.mConfigRoot = new ConfigItemKey();
			this.LoadConfig();
		}
		...
		public override bool InitAppDriver()
		{
			...
			this.mApp.mProductVersion = this.GetProductVersion("");
			...
			this.mApp.mWidth = 640;
			this.mApp.mHeight = 1066;
			...
			this.mApp.mUpdateCount.value = 0;
			...
			this.mApp.mOrigScreenWidth = 640;
			this.mApp.mOrigScreenHeight = 1066;
		...
		public override void Init()
		{
			...
			this.mApp.mWidgetManager.mImage = this.mXNAGraphicsDriver.GetScreenImage();
		...
		public override bool UpdateAppStep(ref bool updated)
		{
			...
			{
				int num = this.mApp.mUpdateCount;
				this.DoUpdateFrames();
				this.DoUpdateFrames();
				this.DoUpdateFramesF(2f);
				updated = this.mApp.mUpdateCount != num;
				updated = true;
			}
			this.mApp.mUpdateAppDepth--;
		...
		{
			bool result = false;
			ConfigItem configItem = this.mConfigRoot[theValueName];
		...
		public override bool ConfigReadData(string theValueName, ref byte[] theValue, ref ulong theLength)
		{
			...
			if (configItem != null && configItem.type == ConfigType.Data)
			{
				...
				theLength = (ulong)((long)configItemData.value.Length);
				result = true;
			}
			return result;
		}
		...
		{
			bool result = false;
			if (this.mConfigRoot.create(theValueName, ConfigType.String))
			{
				ConfigItemString configItemString = (ConfigItemString)this.mConfigRoot[theValueName];
		...
		{
			bool result = false;
			if (this.mConfigRoot.create(theValueName, ConfigType.Int))
			{
				ConfigItemInt configItemInt = (ConfigItemInt)this.mConfigRoot[theValueName];
		...
		{
			bool result = false;
			if (this.mConfigRoot.create(theValueName, ConfigType.Boolean))
			{
				ConfigItemBoolean configItemBoolean = (ConfigItemBoolean)this.mConfigRoot[theValueName];
		...
		{
			bool result = false;
			if (this.mConfigRoot.create(theValueName, ConfigType.Data))
			{
				ConfigItemData configItemData = (ConfigItemData)this.mConfigRoot[theValueName];
		...
		public override void SaveConfig()
		{
			...
			if (this.mConfigRoot != null)
			{
				this.mConfigRoot.save(buffer);
				if (this.WriteBufferToFile(this.configFileName(), buffer))
				{
				}
			}
		}
		...
		{
			Buffer buffer = new Buffer();
			if (this.ReadBufferFromFile(this.configFileName(), buffer, false) && buffer.ReadInt32() == 0)
			{
				this.mConfigRoot = new ConfigItemKey();
				this.mConfigRoot.load(buffer);
			}
		}
		...
		public override bool ReadBufferFromFile(string theFileName, Buffer theBuffer, bool dontWriteToDemo)
		{
			...
			try
			{
				...
				using (IsolatedStorageFileStream isolatedStorageFileStream = userStoreForApplication.OpenFile(theFileName, 3, 1))
				{
		...
		public override bool WriteBytesToFile(string theFileName, byte[] theData, ulong theDataLen)
		{
			...
			{
				IsolatedStorageFile userStoreForApplication = IsolatedStorageFile.GetUserStoreForApplication();
				using (IsolatedStorageFileStream isolatedStorageFileStream = userStoreForApplication.OpenFile(theFileName, 2, 2))
				{
					using (BinaryWriter binaryWriter = new BinaryWriter(isolatedStorageFileStream))
					{
