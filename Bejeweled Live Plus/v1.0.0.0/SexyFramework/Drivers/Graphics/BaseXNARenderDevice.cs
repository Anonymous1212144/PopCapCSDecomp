...
namespace SexyFramework.Drivers.Graphics
{
	public class BaseXNARenderDevice : RenderDevice3D
	{
		public BaseXNARenderDevice(IGraphicsDriver theDriver)
		{
			...
			this.mDevice.IsFullScreen = true;
			...
			this.mBatchedIndexBuffer = new short[BaseXNARenderDevice.mBatchedTriangleSize * 2];
		...
		public override void SetCurrentContext(HRenderContext theContext)
		{
			...
			if (theContext == null || theContext.GetPointer() == null)
			{
				...
				this.mStateMgr.SetProjectionTransform(Matrix.CreateOrthographicOffCenter(0f, 640f, 1066f, 0f, -1000f, 1000f));
				this.SetViewport(0, 0, 480, 800, 0f, 1f);
		...
		public override void DrawPrimitiveEx(uint theVertexFormat, Graphics3D.EPrimitiveType thePrimitiveType, SexyVertex2D[] theVertices, int thePrimitiveCount, Color theColor, int theDrawMode, float tx, float ty, bool blend, uint theFlags)
		{
			...
			this.mStateMgr.PushState();
			Color color;
			color..ctor(theColor.mRed, theColor.mGreen, theColor.mBlue, theColor.mAlpha);
		...
		public override void PushTransform(SexyTransform2D theTransform, bool concatenate)
		{
			...
			{
				this.mTransformStack.Push(theTransform);
				return;
			}
			SexyTransform2D impliedObject = this.mTransformStack.Peek();
			this.mTransformStack.Push(impliedObject * theTransform);
		}
		...
		public void DrawPolyClipped(Rect theClipRect, VertexPositionColorTexture[] theList)
		{
			this.DPC_l2.Clear();
			this.DPC_l1.Clear();
			this.DPC_l1.AddRange(theList);
			int mX = theClipRect.mX;
			int num = mX + theClipRect.mWidth;
			int mY = theClipRect.mY;
			int num2 = mY + theClipRect.mHeight;
			this.ClipPoints(0, (float)mX, BaseXNARenderDevice.ClipperType.Clipper_Less, this.DPC_l1, this.DPC_l2);
			this.DPC_l1.Clear();
			this.ClipPoints(1, (float)mY, BaseXNARenderDevice.ClipperType.Clipper_Less, this.DPC_l2, this.DPC_l1);
			this.DPC_l2.Clear();
			this.ClipPoints(0, (float)num, BaseXNARenderDevice.ClipperType.Clipper_GreaterEqual, this.DPC_l1, this.DPC_l2);
			this.DPC_l1.Clear();
			this.ClipPoints(1, (float)num2, BaseXNARenderDevice.ClipperType.Clipper_GreaterEqual, this.DPC_l2, this.DPC_l1);
			this.CheckBatchAndCommit();
			if (this.DPC_l1.Count >= 3)
			{
				this.BufferedDrawPrimitive(6, this.DPC_l1.Count - 2, this.DPC_l1.ToArray(), (int)this.mDefaultVertexSize, this.mDefaultVertexFVF, Matrix.Identity);
			}
		}
		...
		private void BltTransformed(Image theImage, Rect theClipRect, Color theColor, int theDrawMode, Rect theSrcRect, SexyTransform2D theTransform, bool linearFilter, float theX, float theY, bool center)
		{
			if (!this.PreDraw())
			{
				return;
			...
			if (this.mTransformStack.Count == 0)
			{
				this.BltTransformHelper(theImage, theClipRect, theColor, theDrawMode, theSrcRect, theTransform, linearFilter, theX, theY, center);
				return;
			}
			if (theX != 0f || theY != 0f)
			{
				...
				if (center)
				{
					sexyTransform2D.Translate((float)(-(float)theSrcRect.mWidth) / 2f, (float)(-(float)theSrcRect.mHeight) / 2f);
				}
				sexyTransform2D = theTransform * sexyTransform2D;
				sexyTransform2D.Translate(theX, theY);
				sexyTransform2D = this.mTransformStack.Peek() * sexyTransform2D;
			...
			SexyTransform2D theTransform2 = this.mTransformStack.Peek() * theTransform;
			this.BltTransformHelper(theImage, theClipRect, theColor, theDrawMode, theSrcRect, theTransform2, linearFilter, theX, theY, center);
		}

		public override void BltMatrix(Image theImage, float x, float y, SexyTransform2D theMatrix, Rect theClipRect, Color theColor, int theDrawMode, Rect theSrcRect, bool blend)
		{
		...
		public void BltTrianglesHelper(Image theImage, SexyVertex2D[,] theVertices, int theNumTriangles, Color theColor, int theDrawMode, float tx, float ty, bool blend, Rect theClipRect)
		{
			...
			if (image.mAtlasImage != null)
			{
				...
				for (int i = 0; i < length; i++)
				{
					for (int j = 0; j < 3; j++)
					{
						theVertices[i, j].u = (theVertices[i, j].u * (float)num + (float)image.mAtlasStartX) / (float)memoryImage.mWidth;
						theVertices[i, j].v = (theVertices[i, j].v * (float)num2 + (float)image.mAtlasStartY) / (float)memoryImage.mHeight;
					}
				}
			}
			this.SetupDrawMode(theDrawMode);
			...
			if ((double)xnatextureData.mMaxTotalU <= 1.0 && (double)xnatextureData.mMaxTotalV <= 1.0)
			{
				...
				bool flag2 = theClipRect != Rect.INVALIDATE_RECT && (theClipRect.mX != 0 || theClipRect.mY != 0 || theClipRect.mWidth != this.mScreenWidth || theClipRect.mHeight != this.mScreenHeight);
				this.CheckBatchAndCommit();
				if (flag)
				{
					...
					for (int k = 0; k < theNumTriangles; k++)
					{
						...
						this.mBatchedTriangleBuffer[this.mBatchedTriangleIndex++] = new VertexPositionColorTexture(new Vector3(array[0].x, array[0].y, num3), (theVertices[k, 0].color != Color.Zero) ? this.GetXNAColor(theVertices[k, 0].color) : this.GetXNAColor(theColor), new Vector2(theVertices[k, 0].u * xnatextureData.mMaxTotalU, theVertices[k, 0].v * xnatextureData.mMaxTotalV));
						this.mBatchedTriangleBuffer[this.mBatchedTriangleIndex++] = new VertexPositionColorTexture(new Vector3(array[1].x, array[1].y, num3), (theVertices[k, 1].color != Color.Zero) ? this.GetXNAColor(theVertices[k, 1].color) : this.GetXNAColor(theColor), new Vector2(theVertices[k, 1].u * xnatextureData.mMaxTotalU, theVertices[k, 1].v * xnatextureData.mMaxTotalV));
						this.mBatchedTriangleBuffer[this.mBatchedTriangleIndex++] = new VertexPositionColorTexture(new Vector3(array[2].x, array[2].y, num3), (theVertices[k, 2].color != Color.Zero) ? this.GetXNAColor(theVertices[k, 2].color) : this.GetXNAColor(theColor), new Vector2(theVertices[k, 2].u * xnatextureData.mMaxTotalU, theVertices[k, 2].v * xnatextureData.mMaxTotalV));
						...
						if (!BaseXNARenderDevice.SUPPORT_HW_CLIP && flag2)
						{
							...
							for (int l = 0; l < 3; l++)
							{
								array2[l] = this.mBatchedTriangleBuffer[this.mBatchedTriangleIndex - (3 - l)];
							}
				...
				if (!BaseXNARenderDevice.SUPPORT_HW_CLIP && flag2)
				{
					for (int m = 0; m < theNumTriangles; m++)
					{
						...
						this.mBatchedTriangleBuffer[this.mBatchedTriangleIndex++] = new VertexPositionColorTexture(new Vector3(theVertices[m, 0].x, theVertices[m, 0].y, num3), (theVertices[m, 0].color != Color.Zero) ? this.GetXNAColor(theVertices[m, 0].color) : this.GetXNAColor(theColor), new Vector2(theVertices[m, 0].u * xnatextureData.mMaxTotalU, theVertices[m, 0].v * xnatextureData.mMaxTotalV));
						this.mBatchedTriangleBuffer[this.mBatchedTriangleIndex++] = new VertexPositionColorTexture(new Vector3(theVertices[m, 1].x, theVertices[m, 1].y, num3), (theVertices[m, 1].color != Color.Zero) ? this.GetXNAColor(theVertices[m, 1].color) : this.GetXNAColor(theColor), new Vector2(theVertices[m, 1].u * xnatextureData.mMaxTotalU, theVertices[m, 1].v * xnatextureData.mMaxTotalV));
						this.mBatchedTriangleBuffer[this.mBatchedTriangleIndex++] = new VertexPositionColorTexture(new Vector3(theVertices[m, 2].x, theVertices[m, 2].y, num3), (theVertices[m, 2].color != Color.Zero) ? this.GetXNAColor(theVertices[m, 2].color) : this.GetXNAColor(theColor), new Vector2(theVertices[m, 2].u * xnatextureData.mMaxTotalU, theVertices[m, 2].v * xnatextureData.mMaxTotalV));
						this.AdjustVertsForAtlas(0, ref this.mBatchedTriangleBuffer, this.mBatchedTriangleIndex - 3, 3, 0U, 32, 0);
						if (!BaseXNARenderDevice.SUPPORT_HW_CLIP && flag2)
						{
							...
							for (int n = 0; n < 3; n++)
							{
								array3[n] = this.mBatchedTriangleBuffer[this.mBatchedTriangleIndex - (3 - n)];
							}
							this.mBatchedTriangleIndex -= 3;
				...
				while (num4 < theNumTriangles)
				{
					...
					int num5 = 0;
					int num6 = Math.Min(BaseXNARenderDevice.mBatchedTriangleSize - this.mBatchedTriangleIndex, theNumTriangles - num4);
					while (num5 < num6)
					{
						this.mBatchedTriangleBuffer[this.mBatchedTriangleIndex++] = new VertexPositionColorTexture(new Vector3(theVertices[num4, 0].x, theVertices[num4, 0].y, num3), (theVertices[num4, 0].color != Color.Zero) ? this.GetXNAColor(theVertices[num4, 0].color) : this.GetXNAColor(theColor), new Vector2(theVertices[num4, 0].u * xnatextureData.mMaxTotalU, theVertices[num4, 0].v * xnatextureData.mMaxTotalV));
						this.mBatchedTriangleBuffer[this.mBatchedTriangleIndex++] = new VertexPositionColorTexture(new Vector3(theVertices[num4, 1].x, theVertices[num4, 1].y, num3), (theVertices[num4, 1].color != Color.Zero) ? this.GetXNAColor(theVertices[num4, 1].color) : this.GetXNAColor(theColor), new Vector2(theVertices[num4, 1].u * xnatextureData.mMaxTotalU, theVertices[num4, 1].v * xnatextureData.mMaxTotalV));
						this.mBatchedTriangleBuffer[this.mBatchedTriangleIndex++] = new VertexPositionColorTexture(new Vector3(theVertices[num4, 2].x, theVertices[num4, 2].y, num3), (theVertices[num4, 2].color != Color.Zero) ? this.GetXNAColor(theVertices[num4, 2].color) : this.GetXNAColor(theColor), new Vector2(theVertices[num4, 2].u * xnatextureData.mMaxTotalU, theVertices[num4, 2].v * xnatextureData.mMaxTotalV));
						num5 += 3;
						num4++;
					}
					this.AdjustVertsForAtlas(0, ref this.mBatchedTriangleBuffer, inStartIndex, num5, 0U, 32, 0);
				}
			}
		}
		...
		{
			if (this.mSceneBegun && this.mBatchedTriangleIndex >= 0 && (this.mStateMgr.mStateDirty || this.mStateMgr.mTextureStateDirty || this.mStateMgr.mProjectMatrixDirty))
			{
				if (this.mBatchedTriangleIndex > 0)
				{
		...
		{
			this.mScreenWidth = (this.mDevice.PreferredBackBufferWidth = 640);
			this.mScreenHeight = (this.mDevice.PreferredBackBufferHeight = 1066);
			this.mDevice.PreferredBackBufferWidth = 480;
			this.mDevice.PreferredBackBufferHeight = 800;
			this.mWidth = width;
			this.mHeight = height;
			this.mDevice.PreferMultiSampling = false;
			this.mDevice.SupportedOrientations = 4;
			...
			this.mAdditiveState.AlphaSourceBlend = 4;
			this.mAdditiveState.ColorSourceBlend = 4;
			this.mNormalState.AlphaDestinationBlend = 5;
			this.mNormalState.ColorDestinationBlend = 5;
			this.mNormalState.AlphaSourceBlend = 4;
			this.mNormalState.ColorSourceBlend = 4;
		...
		private void BltClipF(Image theImage, float theX, float theY, Rect theSrcRect, Rect theClipRect, Color theColor, int theDrawMode)
		{
			...
			this.BltTransformed(theImage, theClipRect, theColor, theDrawMode, theSrcRect, theTransform, true, 0f, 0f, false);
		}
		...
		public void BltTransformHelper(Image theImage, Rect theClipRect, Color theColor, int theDrawMode, Rect theSrcRect, SexyTransform2D theTransform, bool linearFilter, float theX, float theY, bool center)
		{
			...
			for (int i = 0; i < 4; i++)
			{
				Vector3.Transform(ref this.mTmpVPCTBuffer[i].Position, ref mMatrix, ref this.mTmpVPCTBuffer[i].Position);
			}
		...
		public Texture2D CreateTexture2D(int theWidth, int theHeight, PixelFormat theFormat, bool renderTarget, XNATextureData theTexData, XNATextureDataPiece[] theTexDataPiece)
		{
			...
			if (renderTarget)
			{
				return new RenderTarget2D(this.mDevice.GraphicsDevice, theWidth, theHeight, false, xnaFormat, 0);
			}
		...
		{
			Matrix result;
			result..ctor(mat.m00, mat.m01, mat.m02, mat.m03, mat.m10, mat.m11, mat.m12, mat.m13, mat.m20, mat.m21, mat.m22, mat.m23, mat.m30, mat.m31, mat.m32, mat.m33);
			return result;
		}
		...
		{
			Matrix result;
			result..ctor(mat.m00, mat.m10, mat.m20, 0f, mat.m01, mat.m11, mat.m21, 0f, 0f, 0f, mat.m22, 0f, mat.m02, mat.m12, 0f, 1f);
			return result;
		}

		public Color GetXNAColor(Color color)
		{
		...
		public void BufferedDrawIndexedPrimitive(int thePrimType, int thePrimCount, VertexPositionColorTexture[] theVertices, int theVertexSize, ulong theVertexFormat, Matrix transform)
		{
			...
			case 5:
			{
				...
				{
					this.mBatchedTriangleBuffer[this.mBatchedTriangleIndex + i] = theVertices[i];
				}
				for (int j = 0; j < thePrimCount / 2; j++)
				{
					this.mBatchedIndexBuffer[this.mBatchedIndexIndex + j * 6] = (short)(this.mBatchedTriangleIndex + num * 4);
					this.mBatchedIndexBuffer[this.mBatchedIndexIndex + j * 6 + 1] = (short)(this.mBatchedTriangleIndex + num * 4 + 1);
					this.mBatchedIndexBuffer[this.mBatchedIndexIndex + j * 6 + 2] = (short)(this.mBatchedTriangleIndex + num * 4 + 2);
					this.mBatchedIndexBuffer[this.mBatchedIndexIndex + j * 6 + 3] = (short)(this.mBatchedTriangleIndex + num * 4 + 3);
					this.mBatchedIndexBuffer[this.mBatchedIndexIndex + j * 6 + 4] = (short)(this.mBatchedTriangleIndex + num * 4 + 2);
					this.mBatchedIndexBuffer[this.mBatchedIndexIndex + j * 6 + 5] = (short)(this.mBatchedTriangleIndex + num * 4 + 1);
				}
				this.mBatchedIndexIndex += thePrimCount * 3;
				this.mBatchedTriangleIndex += theVertices.Length;
				break;
			}
			case 6:
			{
				...
				{
					this.mBatchedTriangleBuffer[this.mBatchedTriangleIndex + k] = theVertices[k];
				}
				int num2 = this.mBatchedTriangleIndex;
				int num3 = num2;
				this.mBatchedTriangleIndex += theVertices.Length;
				this.mBatchedIndexBuffer[this.mBatchedIndexIndex++] = (short)num2;
				this.mBatchedIndexBuffer[this.mBatchedIndexIndex++] = (short)(++num3);
				this.mBatchedIndexBuffer[this.mBatchedIndexIndex++] = (short)(++num3);
				for (int l = 0; l < thePrimCount - 1; l++)
				{
					this.mBatchedIndexBuffer[this.mBatchedIndexIndex + l * 3] = (short)num2;
					this.mBatchedIndexBuffer[this.mBatchedIndexIndex + l * 3 + 1] = (short)(num3 - 1);
					this.mBatchedIndexBuffer[this.mBatchedIndexIndex + l * 3 + 2] = (short)num3;
					num3++;
				}
				this.mBatchedIndexIndex += (thePrimCount - 1) * 3;
				break;
			}
			}
