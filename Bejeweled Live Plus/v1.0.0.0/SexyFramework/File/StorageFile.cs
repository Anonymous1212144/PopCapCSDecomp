...
namespace SexyFramework.File
{
	public class StorageFile
	{
		...
		public static bool ReadBufferFromFile(string theFileName, Buffer theBuffer)
		{
			...
			IsolatedStorageFileStream isolatedStorageFileStream = userStoreForApplication.OpenFile(theFileName, 3);
		...
		{
			IsolatedStorageFile userStoreForApplication = IsolatedStorageFile.GetUserStoreForApplication();
			IsolatedStorageFileStream isolatedStorageFileStream = userStoreForApplication.OpenFile(theFileName, 2);
			if (isolatedStorageFileStream == null)
			{
				return false;
			}
		...
		public bool openRead(string fName, bool bSilent, bool bFromDocs)
		{
			...
			this.fp = this.m_userStore.OpenFile(fName, 3);
			if (this.fp == null)
			{
				return false;
			}
		...
		{
			this.clear();
			this.fp = this.m_userStore.OpenFile(fName, 2);
			if (this.fp == null)
			{
				return false;
			}
		...
		{
			return (int)(this.fp.Seek(0L, (int)this.fp.Length) - 12L);
		}
