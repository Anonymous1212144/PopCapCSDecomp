	...
	public class BonusText
	{
		...
		public void Update()
		{
			...
			if (this.mBulgeDir != 0)
			{
				...
				if (Common._eq(this.mSize, 1f) && Common._leq(this.mBulgePct, 1f))
				{
					this.mSize = 1f;
					this.mBulgeDir = 0;
				}
		...
		public void Draw(Graphics g)
		{
			if (!this.mDone)
			{
				...
				if (!Common._eq(this.mSize, 1f) && graphics3D != null)
				{
					int num4 = 0;
					int num5 = g.GetFont().StringWidth(this.mText);
					int wordWrappedHeight = g.GetWordWrappedHeight(1000000, this.mText, -1, ref num4, ref num4);
					SexyTransform2D theTransform = new SexyTransform2D(false);
					theTransform.Translate((float)(-(float)num3 - num5 / 2 + GlobalMembers.gSexyAppBase.mScreenBounds.mX), (float)(-(float)num2 - wordWrappedHeight / 2));
					theTransform.Scale(this.mSize, this.mSize);
					theTransform.Translate((float)(num3 + num5 / 2 - GlobalMembers.gSexyAppBase.mScreenBounds.mX), (float)(num2 + wordWrappedHeight / 2));
				...
				g.SetColor(color);
				g.WriteWordWrapped(new Rect(num3, num2, 1000000, 100000), this.mText);
				if (this.mImage != null)
				{
					g.PushState();
