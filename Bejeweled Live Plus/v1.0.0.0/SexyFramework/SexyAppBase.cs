...
namespace SexyFramework
{
	public class SexyAppBase : ButtonListener, DialogListener
	{
		...
		public virtual void Dispose()
		{
			...
			if (this.mSharedRTPool != null)
			{
				this.mSharedRTPool.Dispose();
				this.mSharedRTPool = null;
			}
		...
		{
			this.mAppDriver.CopyToClipboard(theString);
		}

		public string GetClipboard()
		{
			return this.mAppDriver.GetClipboard();
		}
		...
		public virtual void ReadFromRegistry()
		{
			...
			this.mRegKey = this.GetString("RegistryKey", string.Empty);
			int num = 0;
		...
		public bool ReadBufferFromStorage(string theFileName, Buffer theBuffer)
		{
			return this.mAppDriver.ReadBufferFromFile(theFileName, theBuffer, false);
		}

		public bool ReadBufferFromFile(string theFileName, Buffer theBuffer)
		{
		...
		protected virtual void ReorderSystemButtonHandler(SystemButtons button, List<Widget> handlers)
		{
		}

		public virtual void UpdateFrames()
		{
			this.mUpdateCount.value++;
			this.mGamepadDriver.Update();
			if (!this.mMinimized)
			{
				if (this.mWidgetManager.UpdateFrame())
				{
					this.mFPSDirtyCount++;
				}
				if (GlobalMembers.IsBackButtonPressed)
				{
					...
					foreach (Widget widget in this.mWidgetManager.mWidgets)
					{
						if (widget.mVisible && !widget.mDisabled)
						{
							list.Add(widget);
						}
					}
					this.ReorderSystemButtonHandler(SystemButtons.Back, list);
					foreach (Widget widget2 in list)
					{
						widget2.SignalSystemButtonPressed(systemButtonPressedArgs);
		...
		public virtual bool LoadProperties(string theFileName, bool required, bool checkSig, bool needsLocaleCorrection)
		{
			...
			if (!this.ReadBufferFromFile(theFileName, buffer))
			{
				bool flag = false;
				if (needsLocaleCorrection && this.mResourceManager != null)
				{
					buffer.Clear();
					flag = this.ReadBufferFromFile(this.mResourceManager.GetLocaleFolder(true) + theFileName, buffer);
				}
				if (!flag)
				{
					if (!required)
		...
		public virtual void SetString(string anID, string value)
		{
			...
			int id = 0;
			if (int.TryParse(anID, ref id))
			{
				this.mPopLoc.SetString(id, value, false);
			}
		}
		...
		public virtual void Init()
		{
			...
			this.mResourceManager = ((XNAFileDriver)this.mFileDriver).GetContentManager().Load<ResourceManager>("properties/resources.xml");
		...
		public SharedRenderTarget.Pool GetSharedRenderTargetPool()
		{
			if (this.mSharedRTPool == null)
			{
				this.mSharedRTPool = new SharedRenderTarget.Pool();
			}
			return this.mSharedRTPool;
		}
				...
				if (this.mCleanupSharedImages)
				{
					foreach (KeyValuePair<KeyValuePair<string, string>, SharedImage> keyValuePair in this.mSharedImageMap)
					{
						...
						if (value.mImage != null && value.mRefCount == 0)
						{
							...
							Dictionary<KeyValuePair<string, string>, SharedImage>.Enumerator enumerator;
							KeyValuePair<KeyValuePair<string, string>, SharedImage> keyValuePair2 = enumerator.Current;
		...
		public ulong HSLToRGB(int h, int s, int l)
		{
			...
			return (ulong)(-16777216) | (ulong)((ulong)((long)num6) << 16) | (ulong)((ulong)((long)num7) << 8) | (ulong)((long)num8);
		}
		...
		public ulong RGBToHSL(int r, int g, int b)
		{
			...
			return (ulong)(-16777216) | (ulong)((long)num3) | (ulong)((long)((long)num4 << 8)) | (ulong)((long)((long)num5 << 16));
		}
		...
		public void HSLToRGB(ulong[] theSource, ulong[] theDest, int theSize)
		{
			for (int i = 0; i < theSize; i++)
			{
				...
				theDest[i] = (num & (ulong)(-16777216)) | (this.HSLToRGB((int)(num & 255UL), (int)(num >> 8) & 255, (int)(num >> 16) & 255) & 16777215UL);
			}
		}
		...
		public void RGBToHSL(ulong[] theSource, ulong[] theDest, int theSize)
		{
			...
			{
				ulong num = theSource[i];
				theDest[i] = (num & (ulong)(-16777216)) | (this.RGBToHSL((int)((num >> 16) & 255UL), (int)(num >> 8) & 255, (int)(num & 255UL)) & 16777215UL);
			}
		}
		...
		public virtual void SetSfxVolume(double theVolume)
		{
			if (this.mSoundManager != null)
			{
				this.mSoundManager.SetVolume((this.mMuteCount > 0) ? 0.0 : theVolume);
			}
		...
		public class Touch
		{
			...
			public Point previousLocation = default(Point);
