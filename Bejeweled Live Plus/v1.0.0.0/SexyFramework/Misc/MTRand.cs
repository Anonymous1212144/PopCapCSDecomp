...
namespace SexyFramework.Misc
{
	public class MTRand
	{
		...
		{
			string[] array = theSerialData.Split(new char[] { '|' });
			if (array.Length == MTRand.MTRAND_N + 1)
			{
				this.mti = int.Parse(array[0]);
				for (int i = 0; i < MTRand.MTRAND_N; i++)
				{
					this.mt[i] = uint.Parse(array[i + 1]);
				}
				return;
			}
			this.SRand(4357U);
		...
		public string Serialize()
		{
			...
			stringBuilder.AppendFormat("{0}", new object[] { this.mti });
			for (int i = 0; i < MTRand.MTRAND_N; i++)
			{
				stringBuilder.AppendFormat("|{0}", new object[] { this.mt[i] });
			}
