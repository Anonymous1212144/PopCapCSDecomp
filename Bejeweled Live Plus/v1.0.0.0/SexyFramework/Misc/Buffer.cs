...
namespace SexyFramework.Misc
{
	public class Buffer
	{
		...
		{
			if (this.mReadBitPos != 0)
			{
				return (this.mReadBitPos + 7) / 8;
			}
			return 0;
		}
		...
		{
			if (this.mWriteBitPos != 0)
			{
				return (this.mWriteBitPos + 7) / 8;
			}
			return 0;
		}
		...
		{
			this.mReadBitPos = pos;
		}

		public void SeekReadByte(int pos)
		{
			this.SeekReadBit(pos * 8);
		}

		public void SeekWriteBit(int pos)
		{
			this.mWriteBitPos = pos;
		}

		public void SeekWriteByte(int pos)
		{
			this.SeekWriteBit(pos * 8);
		}

		public void Seek(Buffer.SeekMode mode)
		{
			this.Seek(mode, 0);
		}

		public void Seek(Buffer.SeekMode mode, int bytePos)
		{
			switch (mode)
			{
			case Buffer.SeekMode.eReadStart:
				this.SeekReadByte(0);
				return;
			case Buffer.SeekMode.eReadForward:
				this.SeekReadByte(this.GetCurrReadBytePos() + bytePos);
				return;
			case Buffer.SeekMode.eReadBack:
				this.SeekReadByte(this.GetCurrReadBytePos() - bytePos);
				return;
			case Buffer.SeekMode.eReadEnd:
				this.SeekReadByte(this.GetDataLen());
				return;
			case Buffer.SeekMode.eWriteStart:
				this.SeekWriteByte(0);
				return;
			case Buffer.SeekMode.eWriteForward:
				this.SeekWriteByte(this.GetCurrWriteBytePos() + bytePos);
				return;
			case Buffer.SeekMode.eWriteBack:
				this.SeekWriteByte(this.GetCurrWriteBytePos() - bytePos);
				return;
			case Buffer.SeekMode.eWriteEnd:
				this.SeekWriteByte(this.GetDataLen());
				return;
			default:
				return;
			}
		...
		public void WriteByte(byte theByte)
		{
			if (this.mWriteBitPos >= this.mDataBitSize)
			{
				...
				else
				{
					...
					List<byte> list;
					int num2;
					(list = this.mData)[num2 = this.mWriteBitPos / 8] = list[num2] | (byte)(theByte << num);
					this.mData.Add((byte)(theByte >> 8 - num));
				}
			}
			else if (this.mData.Count != 0 && this.GetCurrWriteBytePos() < this.mData.Count - 1)
			{
				int currWriteBytePos = this.GetCurrWriteBytePos();
				if (this.mWriteBitPos % 8 == 0)
				{
					this.mData[currWriteBytePos] = theByte;
				}
				else
				{
					int num3 = this.mWriteBitPos % 8;
					List<byte> list2;
					int num4;
					(list2 = this.mData)[num4 = currWriteBytePos] = list2[num4] | (byte)(theByte << num3);
					int num5 = currWriteBytePos + 1;
					if (num5 >= this.mData.Count)
					{
						this.mData.Add((byte)(theByte >> 8 - num3));
					}
					else
					{
						this.mData[num5] = (byte)(theByte >> 8 - num3);
					}
				}
			}
			this.mWriteBitPos += 8;
		...
		public void WriteNumBits(int theNum, int theBits)
		{
			for (int i = 0; i < theBits; i++)
			{
				...
				if ((theNum & (1 << i)) != 0)
				{
					List<byte> list;
					int num;
					(list = this.mData)[num = this.mWriteBitPos / 8] = list[num] | (byte)(1 << this.mWriteBitPos % 8);
				}
				this.mWriteBitPos++;
			}
			if (this.mWriteBitPos > this.mDataBitSize)
		...
		public void WriteString(string theString)
		{
			...
			{
				this.WriteByte((byte)theString.get_Chars(i));
			}
		}
		...
		{
			Buffer.strReader_.Clear();
			int num = (int)this.ReadShort();
			for (int i = 0; i < num; i++)
			{
				Buffer.strReader_.Append((char)this.ReadByte());
			}
			return Buffer.strReader_.ToString();
		}
