...
namespace SexyFramework
{
	public class CurvedVal
	{
		...
		{
			int num = 0;
			num += CurvedVal.CVCharToInt((sbyte)theStr.get_Chars(0));
			num *= 90;
			num += CurvedVal.CVCharToInt((sbyte)theStr.get_Chars(1));
			num *= 90;
			num += CurvedVal.CVCharToInt((sbyte)theStr.get_Chars(2));
		...
		public void CopyFrom(CurvedVal rhs)
		{
			if (this != rhs)
			{
				...
				this.mRamp = rhs.mRamp;
				this.mSingleTrigger = rhs.mSingleTrigger;
				this.mNoClip = rhs.mNoClip;
				this.mOutputSync = rhs.mOutputSync;
				this.mTriggered = rhs.mTriggered;
				this.mIsHermite = rhs.mIsHermite;
				this.mAutoInc = rhs.mAutoInc;
				this.mInitAppUpdateCount = rhs.mInitAppUpdateCount;
				if (GlobalMembers.gSexyAppBase != null)
				{
					this.mAppUpdateCountSrc = GlobalMembers.gSexyAppBase.mUpdateCount;
				}
				else
				{
					this.mAppUpdateCountSrc = null;
				}
				this.mOutMin = rhs.mOutMin;
				this.mOutMax = rhs.mOutMax;
				this.mInMin = rhs.mInMin;
				this.mInMax = rhs.mInMax;
				this.mCurOutVal = rhs.mCurOutVal;
				this.mInVal = rhs.mInVal;
				this.mPrevInVal = rhs.mPrevInVal;
				this.mIncRate = rhs.mIncRate;
				this.mPrevOutVal = rhs.mPrevOutVal;
				this.mCurveCacheRecord = rhs.mCurveCacheRecord;
				this.mLinkedVal = rhs.mLinkedVal;
				this.mDataP = rhs.mDataP;
				this.mCurDataPStr = rhs.mCurDataPStr;
			}
		}
		...
		protected void ParseDataString(string theString)
		{
			...
			{
				num = (int)(theString.get_Chars(0) - 'a');
			}
			i++;
			if (num >= 1)
			{
				int num2 = CurvedVal.CVCharToInt((sbyte)theString.get_Chars(i++));
			...
			double num4 = 0.0;
			double.TryParse(theString.Substring(i, num3 - i), 167, CultureInfo.InvariantCulture, ref num4);
			...
			if (!CurvedVal.mCurveCacheMap.ContainsKey(text))
			{
				...
				{
					sbyte b = (sbyte)theString.get_Chars(i++);
		...
		public double GetOutVal(double theInVal)
		{
			...
			case 1:
				if (this.mMode == 2)
				{
					if (theInVal - this.mInMin <= (this.mInMax - this.mInMin) * 0.5)
					{
			...
			case 2:
			{
				double num = (theInVal - this.mInMin) / (this.mInMax - this.mInMin) * CurvedVal.PI * 0.5;
				if (this.mMode == 2)
				{
					num *= 2.0;
				}
				if (num > CurvedVal.PI * 0.5)
				{
			...
			case 3:
			{
				double num2 = (theInVal - this.mInMin) / (this.mInMax - this.mInMin) * CurvedVal.PI * 0.5;
				if (this.mMode == 2)
				{
					num2 *= 2.0;
				}
			...
			case 4:
			{
				...
				return this.mOutMin + (-Math.Cos(num3) + 1.0) * 0.5 * (this.mOutMax - this.mOutMin);
			}
			case 5:
			{
				...
				if (num4 < CurvedVal.PI * 0.5)
				{
					return this.mOutMin + Math.Sin(num4) * 0.5 * (this.mOutMax - this.mOutMin);
				}
				return this.mOutMin + (2.0 - Math.Sin(num4)) * 0.5 * (this.mOutMax - this.mOutMin);
			}
			case 6:
			{
				this.CheckCurveChange();
		...
		{
			return this.mIncRate != 0.0 && this.IncInVal(this.mIncRate * (double)theScalar);
		}
		...
		public static double PI = 3.1415901184082031;
