...
namespace SexyFramework.Resource
{
	public class ResourceManager
	{
		...
		protected virtual bool ParseSoundResource(XMLElement theElement)
		{
			...
			{
				double.TryParse(theElement.GetAttribute("volume"), 167, CultureInfo.InvariantCulture, ref soundRes.mVolume);
			}
			if (theElement.HasAttribute("pan"))
			{
				int.TryParse(theElement.GetAttribute("pan"), ref soundRes.mPanning);
			}
		...
		protected virtual bool ParseImageResource(XMLElement theElement)
		{
			...
			if (theElement.HasAttribute("anim"))
			{
				...
				if (string.Compare(attribute3, "none", 5) == 0)
				{
					animType = AnimType.AnimType_None;
				}
				else if (string.Compare(attribute3, "once", 5) == 0)
				{
					animType = AnimType.AnimType_Once;
				}
				else if (string.Compare(attribute3, "loop", 5) == 0)
				{
					animType = AnimType.AnimType_Loop;
				}
				else
				{
					if (string.Compare(attribute3, "pingpong", 5) != 0)
					{
				...
				for (;;)
				{
					...
					if (xmlelement.mType == XMLElement.XMLElementType.TYPE_START)
					{
						if (xmlelement.mValue.ToString() == "Resources")
						{
							...
							this.mCurResGroupLocSet = (uint)((attribute2.Length < 4) ? '\0' : (((uint)attribute2.get_Chars(0) << 24) | ((uint)attribute2.get_Chars(1) << 16) | ((uint)attribute2.get_Chars(2) << 8) | attribute2.get_Chars(3)));
						...
						else
						{
							...
							for (;;)
							{
								...
								if (xmlelement2.mType == XMLElement.XMLElementType.TYPE_START)
								{
									...
									{
										mLocSet = (uint)(((uint)attribute6.get_Chars(0) << 24) | ((uint)attribute6.get_Chars(1) << 16) | ((uint)attribute6.get_Chars(2) << 8) | attribute6.get_Chars(3));
									}
		...
		public virtual void DeleteResources(Dictionary<string, BaseRes> theMap, string theGroup)
		{
			if (theGroup.Length <= 0)
			{
				foreach (KeyValuePair<string, BaseRes> keyValuePair in theMap)
				{
					if (keyValuePair.Value.mDirectLoaded)
					{
						...
						keyValuePair2.Value.mDirectLoaded = false;
						KeyValuePair<string, BaseRes> keyValuePair3 = enumerator.Current;
						this.Deref(keyValuePair3.Value);
					}
				}
				return;
			}
					...
					while (enumerator2.MoveNext())
					{
						...
						if (subGroup.mGroupName.Length > 0)
						{
							foreach (KeyValuePair<string, BaseRes> keyValuePair4 in theMap)
							{
								...
								{
									Dictionary<string, BaseRes>.Enumerator enumerator3;
									KeyValuePair<string, BaseRes> keyValuePair5 = enumerator3.Current;
									if (keyValuePair5.Value.mResGroup == subGroup.mGroupName)
									{
										KeyValuePair<string, BaseRes> keyValuePair6 = enumerator3.Current;
										keyValuePair6.Value.mDirectLoaded = false;
										KeyValuePair<string, BaseRes> keyValuePair7 = enumerator3.Current;
										this.Deref(keyValuePair7.Value);
									}
								}
							}
						}
			...
			foreach (KeyValuePair<string, BaseRes> keyValuePair8 in theMap)
			{
				if (keyValuePair8.Value.mDirectLoaded)
				{
					...
					KeyValuePair<string, BaseRes> keyValuePair9 = enumerator4.Current;
					if (keyValuePair9.Value.mResGroup == theGroup)
					{
						KeyValuePair<string, BaseRes> keyValuePair10 = enumerator4.Current;
						keyValuePair10.Value.mDirectLoaded = false;
						KeyValuePair<string, BaseRes> keyValuePair11 = enumerator4.Current;
						this.Deref(keyValuePair11.Value);
					}
				}
			}
		}
		...
		public BaseRes GetBaseRes(int theType, string theId)
		{
			if (this.mCurArtResKey.Length <= 0)
			{
				...
				this.mCurLocSetKey = "||" + string.Format("{0:x}", this.mCurLocSet);
				this.mCurArtResAndLocSetKey = this.mCurArtRes + "|||" + string.Format("{0:x}", this.mCurLocSet);
			}
		...
		public virtual bool DoLoadImage(ImageRes theRes)
		{
			lock (this.mLoadCrit)
			{
				...
				{
					bool flag2 = theRes.mAtlasName != null;
					bool flag3 = false;
					...
					if (!flag3)
					{
						sharedImageRef2 = this.mApp.GetSharedImage(mPath, theRes.mVariant, ref flag4, !theRes.mNoTriRep, flag2);
					}
		...
		public virtual bool DoLoadPopAnim(PopAnimRes theRes)
		{
			...
			lock (this.mLoadCrit)
			{
				...
				{
					popAnim.mImgScale = (float)this.mCurArtRes / (float)this.mBaseArtRes;
					popAnim.mDrawScale = (float)this.mCurArtRes / (float)this.mBaseArtRes;
				}
				...
				else
				{
					if (flag2)
					{
						FRect frect = new FRect((float)popAnim.mAnimRect.mX, (float)popAnim.mAnimRect.mY, (float)popAnim.mAnimRect.mWidth, (float)popAnim.mAnimRect.mHeight);
						frect.Scale(0.34999999403953552, 0.34999999403953552);
		...
		{
			return (this.GetImage(theId) != null && this.GetImage(theId).GetDeviceImage() != null) || this.GetFont(theId) != null || this.GetSound(theId) != -1;
		}
		...
		{
			theName == "IMAGE_UI_MAINMENU_TIKI";
			SharedImageRef result;
			lock (this.mLoadCrit)
			{
				...
				else
				{
					...
					{
						result = imageRes.mImage;
					}
					else if (imageRes.mFromProgram)
					{
						result = null;
					}
					else if (!this.DoLoadImage(imageRes))
					{
						result = null;
					}
					else
		...
		public Font LoadFont(string theName)
		{
			...
			lock (this.mLoadCrit)
			{
				...
				else
				{
					...
					{
						result = fontRes.mFont;
					}
					else if (fontRes.mFromProgram)
					{
						result = null;
					}
					else if (!this.DoLoadFont(fontRes))
					{
						result = null;
					}
					else
		...
		public int LoadSound(string theName)
		{
			...
			lock (this.mLoadCrit)
			{
				...
				else
				{
					...
					{
						result = soundRes.mSoundId;
					}
					else if (soundRes.mFromProgram)
					{
						result = -1;
					}
					else if (!this.DoLoadSound(soundRes))
					{
						result = -1;
					}
					else
		...
		public PopAnim LoadPopAnim(string theName)
		{
			...
			lock (this.mLoadCrit)
			{
				...
				else
				{
					...
					{
						result = popAnimRes.mPopAnim;
					}
					else if (popAnimRes.mFromProgram)
					{
						result = null;
					}
					else if (!this.DoLoadPopAnim(popAnimRes))
					{
						result = null;
					}
					else
		...
		public string GetIdByPath(string thePath)
		{
			...
			for (int i = 0; i < 7; i++)
			{
				foreach (KeyValuePair<string, BaseRes> keyValuePair in this.mResMaps[i])
				{
					if (keyValuePair.Value.mPath == text)
					{
						Dictionary<string, BaseRes>.Enumerator enumerator;
						KeyValuePair<string, BaseRes> keyValuePair2 = enumerator.Current;
						return keyValuePair2.Value.mId;
					}
				}
			}
			text = text.ToUpper();
			for (int j = 0; j < 7; j++)
			{
				foreach (KeyValuePair<string, BaseRes> keyValuePair3 in this.mResMaps[j])
				{
					...
					{
						Dictionary<string, BaseRes>.Enumerator enumerator2;
						KeyValuePair<string, BaseRes> keyValuePair4 = enumerator2.Current;
						return keyValuePair4.Value.mId;
					}
				}
			}
			return "";
		...
		public virtual int GetSoundThrow(string theId)
		{
			...
			SoundRes soundRes = (SoundRes)this.GetBaseRes(1, theId);
			soundRes.mSoundId = this.GetSound(theId);
			if (soundRes != null)
			{
				if (soundRes.mSoundId != -1)
				{
				...
				if (this.mAllowMissingProgramResources && soundRes.mFromProgram)
				{
					return -1;
				}
			}
		...
		public virtual Font GetFontThrow(string theId, int artRes)
		{
			...
			if (artRes != 0 && artRes != this.mCurArtRes)
			{
				this.Fail(string.Concat(new object[] { "Attempted to load font of incorrect art resolution ", artRes, " (expected ", this.mCurArtRes, "):", theId }));
				throw new ResourceManagerException(this.GetErrorText());
			}
			FontRes fontRes = (FontRes)this.GetBaseRes(2, theId);
			if (fontRes != null)
			{
				...
				if (this.mAllowMissingProgramResources && fontRes.mFromProgram)
				{
					return null;
				}
			}
		...
		public virtual PIEffect GetPIEffectThrow(string theId)
		{
			...
			if (pieffectRes != null)
			{
				...
				if (this.mAllowMissingProgramResources && pieffectRes.mFromProgram)
				{
					return null;
				}
			}
			this.Fail(string.Format("PIEffect resource not found: {0}", theId));
			return null;
		}
		...
		public Point? GetOffsetOfImage(string theId)
		{
			...
			return new Point?(imageRes.mOffset);
		}
		...
		public ResourceRef GetImageRef(Image theGlobalPtrRef)
		{
			...
			return this.GetResourceRef(0, mNameForRes);
		}
		...
		public float GetLoadResourcesListProgress(string[] theGroups)
		{
			...
			{
				num += groupLoadInfo.mTotalFile;
				num2 += groupLoadInfo.mCurrentFile;
			}
			if (num == 0)
			{
				return 0f;
			}
			if (!this.mLoadFinished && num2 != num)
			{
		...
		public void LoadGroupAsyn(string[] group)
		{
			...
			this.GetLoadingGroup(group);
			this.mIsLoading = true;
			this.mLoadSuccess = false;
			this.mLoadingProc = new ThreadStart(this.LoadingProc);
			this.mLoadingThread = new Thread(this.mLoadingProc);
			this.mLoadingThread.Name = "ResLoadingThread";
		...
		private void GetLoadingGroup(string[] group)
		{
			...
			foreach (string text in group)
			{
				if (text != null)
				{
					if (this.mCompositeResGroupMap.ContainsKey(text))
					{
						CompositeResGroup compositeResGroup = this.mCompositeResGroupMap[text];
						int count = compositeResGroup.mSubGroups.Count;
						for (int j = 0; j < count; j++)
						{
							...
							if (subGroup.mGroupName.Length > 0 && (subGroup.mArtRes == 0 || subGroup.mArtRes == this.mCurArtRes) && (subGroup.mLocSet == 0U || subGroup.mLocSet == this.mCurLocSet))
							{
								this.mCurCompositeSubGroupIndex = j;
					...
					else if (this.mResGroupMap.ContainsKey(text))
					{
						ResGroup resGroup2 = this.mResGroupMap[text];
						this.mGroupToLoad.Add(new ResourceManager.GroupLoadInfo(text, resGroup2.mResList.Count));
					}
				}
			}
		}
		...
		private void LoadingProc()
		{
			foreach (ResourceManager.GroupLoadInfo groupLoadInfo in this.mGroupToLoad)
			{
				this.StartLoadResources(groupLoadInfo.mName, false);
				while (this.LoadNextResource())
				{
					groupLoadInfo.mCurrentFile++;
				}
				if (this.HadError())
				{
					this.mLoadSuccess = false;
				...
				this.mLoadedGroups.Add(this.mCurResGroup);
