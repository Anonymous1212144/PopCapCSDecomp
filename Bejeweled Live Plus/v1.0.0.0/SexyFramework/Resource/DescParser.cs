...
namespace SexyFramework.Resource
{
	public abstract class DescParser : EncodingParser
	{
		...
		{
			return (theString.get_Chars(0) >= '0' && theString.get_Chars(0) <= '9') || theString.get_Chars(0) == '-' || theString.get_Chars(0) == '+' || theString.get_Chars(0) == '\'' || theString.get_Chars(0) == '"';
		}

		public string Unquote(string theQuotedString)
		{
			if (theQuotedString.get_Chars(0) == '\'' || theQuotedString.get_Chars(0) == '"')
			{
				char c = theQuotedString.get_Chars(0);
				...
				for (int i = 1; i < theQuotedString.Length - 1; i++)
				{
					if (flag2)
					{
						char c2 = theQuotedString.get_Chars(i);
						char c3 = c2;
						if (c3 != 'n')
						{
							if (c3 == 't')
					...
					else if (theQuotedString.get_Chars(i) == c)
					{
						if (flag)
						{
					...
					else if (theQuotedString.get_Chars(i) == '\\')
					{
						flag2 = true;
						flag = false;
					}
					...
					{
						stringBuilder.Append(theQuotedString.get_Chars(i));
						flag = false;
					}
				}
		...
		public bool ParseToList(string theString, ref ListDataElement theList, bool expectListEnd, ref int theStringPos)
		{
			...
			{
				bool flag5 = false;
				char c = theString.get_Chars(theStringPos++);
				bool flag6 = c == ' ' || c == '\t' || c == '\n' || c == ',';
				if (flag3)
				{
					flag5 = true;
