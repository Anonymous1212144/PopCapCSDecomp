...
namespace SexyFramework
{
	public static class Common
	{
		...
		public static bool StringToDouble(string aTempString, ref double theDouble)
		{
			...
			try
			{
				theDouble = double.Parse(aTempString, 167, CultureInfo.InvariantCulture);
				return true;
			}
			catch (Exception)
			{
			...
			for (int i = 0; i < theString.Length; i++)
			{
				...
				if (c == '&')
				{
					...
					if (num != -1)
					{
						...
						else if (text.get_Chars(0) == '#' && text.Length > 1)
						{
							int num2 = (int)c;
							if (text.get_Chars(1) == 'x')
							{
		...
		public static string RemoveTrailingSlash(string theDirectory)
		{
			...
			if (length > 0 && (theDirectory.get_Chars(length - 1) == '\\' || theDirectory.get_Chars(length - 1) == '/'))
			{
		...
		public static string GetPathFrom(string theRelPath, string theDir)
		{
			string text = "";
			string text2 = theDir;
			if (theRelPath.Length >= 2 && theRelPath.get_Chars(1) == ':')
			{
				return theRelPath;
			}
			char c = '/';
			...
			if (text2.Length >= 2 && text2.get_Chars(1) == ':')
			{
				text = text2.Substring(0, 2);
				text2 = text2.Remove(0, 2);
			}
			if (text2.Length > 0 && text2.get_Chars(text2.Length - 1) != '\\' && text2.get_Chars(text2.Length - 1) != '/')
			{
				text2 += c;
			}
		...
		{
			int num = 0;
			while (num < theString.Length && Common.isSpace(theString.get_Chars(num)))
			{
				num++;
			}
			int num2 = theString.Length - 1;
			while (num2 >= 0 && Common.isSpace(theString.get_Chars(num2)))
			{
				num2--;
			}
		...
		public static float FloatRange(float min_val, float max_val)
		{
			...
			{
				return min_val + (float)(Common.SafeRand() % (int)((Math.Abs(min_val) - Math.Abs(max_val)) * 1E+08f + 1f)) / 1E+08f;
			}
			return min_val + (float)(Common.SafeRand() % (int)((max_val - min_val) * 1E+08f + 1f)) / 1E+08f;
		}
		...
		public static bool RotatedRectsIntersect(Rect r1, float r1_angle, Rect r2, float r2_angle)
		{
			...
			Point p = new Point(r2.mWidth / 2, r2.mHeight / 2);
			Point p2 = new Point(r1.mX + point.mX, r1.mY + point.mY);
			Point theTPoint = new Point(r2.mX + p.mX, r2.mY + p.mY);
			Point point2 = new Point(theTPoint);
			point2 -= p2;
			Common._RotatePointClockwise(point2, r2_angle);
			Point impliedObject = new Point(point2);
			Point impliedObject2 = new Point(point2);
			impliedObject -= p;
			impliedObject2 += p;
			Point point3 = default(Point);
			Point point4 = default(Point);
			point3.mX = (int)((float)(-(float)point.mY) * num3);
			point4.mX = point3.mX;
			float num4 = (float)point.mX * num2;
			point3.mX += (int)num4;
			point4.mX -= (int)num4;
			point3.mY = (int)((float)point.mY * num2);
			point4.mY = point3.mY;
			...
			if (num4 < 0f)
			{
				num4 = (float)point3.mX;
				point3.mX = point4.mX;
				point4.mX = (int)num4;
				num4 = (float)point3.mY;
				point3.mY = point4.mY;
				point4.mY = (int)num4;
			}
			if (num3 < 0f)
			...
			if (point4.mX > impliedObject2.mX || point4.mX > -impliedObject.mX)
			{
				return false;
			}
			float num5;
			...
			if (num4 == 0f)
			{
				num5 = (float)point3.mY;
				num6 = -num5;
			}
			else
			{
				float num7 = (float)(impliedObject.mX - point3.mX);
				float num8 = (float)(impliedObject2.mX - point3.mX);
				num5 = (float)point3.mY;
				if (num8 * num7 > 0f)
				{
					float num9 = (float)point3.mX;
					if (num7 < 0f)
					{
						num9 -= (float)point4.mX;
						num5 -= (float)point4.mY;
						num7 = num8;
					}
					else
					{
				...
				num7 = (float)(impliedObject.mX + point3.mX);
				num8 = (float)(impliedObject2.mX + point3.mX);
				num6 = (float)(-(float)point3.mY);
				if (num8 * num7 > 0f)
				{
					float num9 = (float)(-(float)point3.mX);
					if (num7 < 0f)
					{
						num9 -= (float)point4.mX;
						num6 -= (float)point4.mY;
						num7 = num8;
					}
					else
					{
					...
					num6 *= num7;
					num6 /= num9;
					num6 -= (float)point3.mY;
				}
			}
			return (num5 >= (float)impliedObject.mY || num6 >= (float)impliedObject.mY) && (num5 <= (float)impliedObject2.mY || num6 <= (float)impliedObject2.mY);
		}
		...
		public static float JL_PI = 3.14159274f;
