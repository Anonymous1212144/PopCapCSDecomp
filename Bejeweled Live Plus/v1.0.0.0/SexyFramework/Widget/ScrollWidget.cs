	...
	public class ScrollWidget : Widget, ProxyWidgetListener, IDisposable
	{
		...
		public override void TouchMoved(SexyAppBase.Touch touch)
		{
			...
			else if (this.mClientLastDown != null)
			{
				...
				touch.previousLocation.mY = touch.previousLocation.mY + p.mY;
			...
			if (this.mVisible && !this.mDisabled)
			{
				...
				else
				{
					...
					if (this.mScrollOffset.mX < num)
					{
						...
						{
							this.mScrollOffset.mX = this.mScrollOffset.mX + ScrollWidget.SCROLL_SPRINGBACK_TENSION * (num - this.mScrollOffset.mX);
						}
						else
						{
					...
					else if (this.mScrollOffset.mX > num3)
					{
						if (this.mBounceEnabled || this.mSeekScrollTarget)
						{
							this.mScrollOffset.mX = this.mScrollOffset.mX + ScrollWidget.SCROLL_SPRINGBACK_TENSION * (num3 - this.mScrollOffset.mX);
						}
						else
						{
					...
					if (this.mScrollOffset.mY < num2)
					{
						if (this.mBounceEnabled || this.mSeekScrollTarget)
						{
							this.mScrollOffset.mY = this.mScrollOffset.mY + ScrollWidget.SCROLL_SPRINGBACK_TENSION * (num2 - this.mScrollOffset.mY);
						}
						else
						{
					...
					else if (this.mScrollOffset.mY > num4)
					{
						if (this.mBounceEnabled || this.mSeekScrollTarget)
						{
							this.mScrollOffset.mY = this.mScrollOffset.mY + ScrollWidget.SCROLL_SPRINGBACK_TENSION * (num4 - this.mScrollOffset.mY);
						}
						else
						{
		...
		protected void TouchMotion(SexyAppBase.Touch touch)
		{
			...
			FPoint fpoint = new FPoint((float)touch.location.mX, (float)touch.location.mY) - this.mScrollTouchReference;
			FPoint fpoint2 = new FPoint(this.mScrollOffset);
			if ((this.mScrollPractical & ScrollWidget.ScrollMode.SCROLL_HORIZONTAL) != ScrollWidget.ScrollMode.SCROLL_DISABLED)
			{
				...
				else
				{
					...
					float num5 = (float)(num4 * num3 + (1.0 - num4) * (double)this.mScrollVelocity.mX);
					if (!float.IsNaN(num5))
					{
						this.mScrollVelocity.mX = num5;
					}
				}
			}
			if ((this.mScrollPractical & ScrollWidget.ScrollMode.SCROLL_VERTICAL) != ScrollWidget.ScrollMode.SCROLL_DISABLED)
			{
				...
				{
					float num6 = fpoint2.mY - this.mScrollOffset.mY;
					double num7 = touch.timestamp - this.mScrollLastTimestamp;
					double num8 = (double)num6 / num7;
					double num9 = Math.Min(1.0, num7 / (double)ScrollWidget.SCROLL_VELOCITY_FILTER_WINDOW);
					if (double.IsNaN(num8))
					{
						num8 = 0.0;
					}
					float num10 = (float)(num9 * num8 + (1.0 - num9) * (double)this.mScrollVelocity.mY);
					if (!float.IsNaN(num10))
					{
						this.mScrollVelocity.mY = num10;
					}
				}
			}
