	...
	public class PopAnim : Widget
	{
		...
		internal static bool WildcardReplace(string theValue, string theWildcard, string theReplacement, ref string theResult)
		{
			...
			if (theWildcard.get_Chars(0) == '*')
			{
				...
				if (theWildcard.get_Chars(theWildcard.Length - 1) == '*')
				{
					...
					for (int i = 0; i <= num2; i++)
					{
						...
						for (int j = 0; j < num; j++)
						{
							if (char.ToUpper(theWildcard.get_Chars(j + 1)) != char.ToUpper(theValue.get_Chars(i + j)))
							{
								flag = false;
								break;
							}
			...
			else if (theWildcard.get_Chars(theWildcard.Length - 1) == '*')
			{
				if (theValue.Length < theWildcard.Length - 1)
				{
					return false;
		...
		public string Remap(string theString)
		{
			foreach (KeyValuePair<string, string> keyValuePair in this.mRemapList)
			{
				...
				LinkedList<KeyValuePair<string, string>>.Enumerator enumerator;
				KeyValuePair<string, string> keyValuePair2 = enumerator.Current;
				if (PopAnim.WildcardReplace(theString, value, keyValuePair2.Key, ref this.Remap_aString))
				{
		...
		public void FrameHit(PASpriteInst theSpriteInst, PAFrame theFrame, PAObjectPos theObjectPos)
		{
			...
			for (int k = 0; k < theFrame.mCommandVector.Length; k++)
			{
				...
				if (this.mListener == null || !this.mListener.PopAnimCommand(this.mId, theSpriteInst, pacommand.mCommand, pacommand.mParam))
				{
					...
					else if (string.Compare(pacommand.mCommand, "playsample", 5) == 0)
					{
						...
						while (text.Length > 0)
						{
							...
							else
							{
								...
								{
									theVolume = double.Parse(text2.Substring(7), 167, CultureInfo.InvariantCulture);
								}
								...
								{
									theNumSteps = double.Parse(text2.Substring(6), 167, CultureInfo.InvariantCulture);
								}
							}
							if (num4 == -1)
							{
					...
					else if (string.Compare(pacommand.mCommand, "addparticleeffect", 5) == 0)
					{
						...
						while (text3.Length > 0)
						{
							...
							else
							{
								...
								{
									paparticleEffect.mXOfs = double.Parse(text5.Substring(2), 167, CultureInfo.InvariantCulture);
								}
								else if (text5.StartsWith("y="))
								{
									paparticleEffect.mYOfs = double.Parse(text5.Substring(2), 167, CultureInfo.InvariantCulture);
								}
								else if (string.Compare(text5, "attachemitter", 5) == 0)
								{
									paparticleEffect.mAttachEmitter = true;
								}
								else if (string.Compare(text5, "once", 5) == 0)
								{
									flag2 = true;
								}
								else if (string.Compare(text5, "behind", 5) == 0)
								{
									paparticleEffect.mBehind = true;
								}
								else if (string.Compare(text5, "transform", 5) == 0)
								{
						...
						string text6 = Common.GetPathFrom(text4 + "\\" + text4, Common.GetFileDir(this.mLoadedPamFile, false));
						text6 = "images\\960\\help\\" + text4 + "\\" + text4;
						if (this.mListener != null)
						{
						...
						{
							ResourceRef resourceRef = GlobalMembers.gSexyAppBase.mResourceManager.GetResourceRefFromPath(text6 + ".ppf");
							if (resourceRef == null)
							{
						...
						if (paparticleEffect.mEffect == null)
						{
							...
							if (!paparticleEffect.mEffect.LoadEffect(text6 + ".ppf") && !paparticleEffect.mEffect.LoadEffect(pathFrom + ".ppf") && !paparticleEffect.mEffect.LoadEffect(text6 + ".ip3"))
							{
								bool flag4 = false;
		...
		public void CalcObjectPos(PASpriteInst theSpriteInst, int theObjectPosIdx, bool frozen, ref PATransform theTransform, out Color theColor)
		{
			...
			for (int i = 0; i < 3; i++)
			{
				this.COP_aNextObjectPos[i] = null;
			}
			this.COP_anOfsTab[0] = theSpriteInst.mDef.mFrames.Length - 1;
			this.COP_anOfsTab[1] = 1;
			this.COP_anOfsTab[2] = 2;
			if (theSpriteInst == this.mMainSpriteInst && theSpriteInst.mFrameNum >= (float)theSpriteInst.mDef.mWorkAreaStart)
			{
				this.COP_anOfsTab[0] = theSpriteInst.mDef.mWorkAreaDuration - 1;
			}
			...
			if (this.mInterpolate && !frozen)
			{
				for (int j = 0; j < 3; j++)
				{
					PAFrame paframe2 = theSpriteInst.mDef.mFrames[((int)theSpriteInst.mFrameNum + this.COP_anOfsTab[j]) % theSpriteInst.mDef.mFrames.Length];
					if (theSpriteInst == this.mMainSpriteInst && theSpriteInst.mFrameNum >= (float)theSpriteInst.mDef.mWorkAreaStart)
					{
						paframe2 = theSpriteInst.mDef.mFrames[((int)theSpriteInst.mFrameNum + this.COP_anOfsTab[j] - theSpriteInst.mDef.mWorkAreaStart) % (theSpriteInst.mDef.mWorkAreaDuration + 1) + theSpriteInst.mDef.mWorkAreaStart];
					}
					else
					{
						paframe2 = theSpriteInst.mDef.mFrames[((int)theSpriteInst.mFrameNum + this.COP_anOfsTab[j]) % theSpriteInst.mDef.mFrames.Length];
					}
					if (paframe.mHasStop)
					{
						paframe2 = paframe;
					...
					{
						this.COP_aNextObjectPos[j] = paframe2.mFrameObjectPosVector[theObjectPosIdx];
						if (this.COP_aNextObjectPos[j].mObjectNum != paobjectPos.mObjectNum)
						{
							this.COP_aNextObjectPos[j] = null;
						}
					}
					if (this.COP_aNextObjectPos[j] == null)
					{
						for (int k = 0; k < paframe2.mFrameObjectPosVector.Length; k++)
						{
							...
							{
								this.COP_aNextObjectPos[j] = paframe2.mFrameObjectPosVector[k];
								break;
							}
						}
					}
				}
				if (this.COP_aNextObjectPos[1] != null)
				{
					...
					SexyVector2 sexyVector2 = this.COP_aNextObjectPos[1].mTransform.mMatrix * new SexyVector2(0f, 0f);
					if (this.COP_aNextObjectPos[0] != null && this.COP_aNextObjectPos[2] != null)
					{
						SexyVector2 v = this.COP_aNextObjectPos[0].mTransform.mMatrix * new SexyVector2(0f, 0f);
						SexyVector2 impliedObject = this.COP_aNextObjectPos[2].mTransform.mMatrix * new SexyVector2(0f, 0f);
					...
					paobjectPos.mTransform.InterpolateTo(this.COP_aNextObjectPos[1].mTransform, num, ref patransform);
					color = new Color((int)((float)PopAnim.ClrR(paobjectPos.mColorInt) * (1f - num) + (float)PopAnim.ClrR(this.COP_aNextObjectPos[1].mColorInt) * num + 0.5f), (int)((float)PopAnim.ClrG(paobjectPos.mColorInt) * (1f - num) + (float)PopAnim.ClrG(this.COP_aNextObjectPos[1].mColorInt) * num + 0.5f), (int)((float)PopAnim.ClrB(paobjectPos.mColorInt) * (1f - num) + (float)PopAnim.ClrB(this.COP_aNextObjectPos[1].mColorInt) * num + 0.5f), (int)((float)PopAnim.ClrA(paobjectPos.mColorInt) * (1f - num) + (float)PopAnim.ClrA(this.COP_aNextObjectPos[1].mColorInt) * num + 0.5f));
				}
				else
				{
		...
		public void UpdateTransforms(PASpriteInst theSpriteInst, PATransform? theTransform, Color theColor, bool parentFrozen)
		{
			...
			int num = 0;
			foreach (PAObjectPos paobjectPos in paframe.mFrameObjectPosVector)
			{
				if (paobjectPos.mIsSprite)
				{
					Color theColor2;
					this.CalcObjectPos(theSpriteInst, num, flag, ref patransform, out theColor2);
					if (theTransform != null)
					{
						theTransform.Value.TransformSrc(patransform, ref patransform);
					}
					this.UpdateTransforms(theSpriteInst.mChildren[paobjectPos.mObjectNum].mSpriteInst, new PATransform?(patransform), theColor2, flag);
				}
				num++;
			}
			foreach (PAParticleEffect paparticleEffect in theSpriteInst.mParticleEffectVector)
			{
				if (paparticleEffect.mAttachEmitter)
				{
					if (paparticleEffect.mTransform)
					{
						...
						theMatrix.CopyFrom(theSpriteInst.mCurTransform.mMatrix * theMat);
						paparticleEffect.mEffect.mEmitterTransform.CopyFrom(theMatrix);
					}
					else
					{
						...
						paparticleEffect.mEffect.mEmitterTransform.CopyFrom(theMatrix2);
					}
		...
		public void CleanParticles(PASpriteInst theSpriteInst, bool force)
		{
			...
			foreach (PAObjectInst paobjectInst in theSpriteInst.mChildren)
			{
				PASpriteInst mSpriteInst = paobjectInst.mSpriteInst;
				if (mSpriteInst != null)
				{
		...
		public void IncSpriteInstFrame(PASpriteInst theSpriteInst, PAObjectPos theObjectPos, float theFrac)
		{
			...
			{
				theSpriteInst.mFrameRepeats++;
				theSpriteInst.mFrameNum -= (float)theSpriteInst.mDef.mFrames.Length;
			}
		...
		public void DrawParticleEffects(Graphics g, PASpriteInst theSpriteInst, PATransform? theTransform, Color theColor, bool front)
		{
			if (theSpriteInst.mParticleEffectVector.Count > 0)
			{
				for (int i = 0; i < theSpriteInst.mParticleEffectVector.Count; i++)
				{
					...
					if (paparticleEffect.mTransform)
					{
						if (!paparticleEffect.mAttachEmitter)
						{
							...
							theMat.Translate((float)paparticleEffect.mEffect.mWidth / 2f, (float)paparticleEffect.mEffect.mHeight / 2f);
							SexyTransform2D theMatrix = new SexyTransform2D(false);
							theMatrix.CopyFrom(theTransform.Value.mMatrix * theMat);
		...
		public virtual void DrawSprite(Graphics g, PASpriteInst theSpriteInst, PATransform? theTransform, Color theColor, bool additive, bool parentFrozen)
		{
			this.DrawParticleEffects(g, theSpriteInst, theTransform, theColor, false);
			PAFrame paframe = theSpriteInst.mDef.mFrames[(int)theSpriteInst.mFrameNum];
			PATransform patransform = default(PATransform);
			bool flag = parentFrozen || theSpriteInst.mDelayFrames > 0 || paframe.mHasStop;
			for (int i = 0; i < paframe.mFrameObjectPosVector.Length; i++)
			{
				PAObjectPos paobjectPos = paframe.mFrameObjectPosVector[i];
				...
				{
					paobjectInst.mPredrawCallback = this.mListener.PopAnimObjectPredraw(this.mId, g, theSpriteInst, paobjectInst, theTransform.Value, theColor);
				}
				Color mCurColor;
				if (paobjectPos.mIsSprite)
				{
					...
					patransform = new PATransform(true);
					patransform.CopyFrom(mSpriteInst.mCurTransform);
				}
				else
				{
					this.CalcObjectPos(theSpriteInst, i, flag, ref patransform, out mCurColor);
				}
				...
				else if (theTransform == null || paobjectPos.mIsSprite)
				{
					...
					{
						PATransform patransform4 = new PATransform(true);
				...
				{
					theTransform.Value.TransformSrc(patransform, ref patransform2);
				}
				...
				if (color.mAlpha != 0)
				{
					if (paobjectPos.mIsSprite)
					{
						...
						this.DrawSprite(g, mSpriteInst2, new PATransform?(patransform2), color, paobjectPos.mIsAdditive || additive, flag);
					}
					else
					{
						int num = 0;
						for (;;)
						{
							...
							PATransform patransform5 = new PATransform(true);
							...
							if (this.mImgScale != 1f)
							{
								...
								PATransform patransform6 = new PATransform(true);
					...
					{
						paobjectInst.mPostdrawCallback = this.mListener.PopAnimObjectPostdraw(this.mId, g, theSpriteInst, paobjectInst, theTransform.Value, theColor);
					}
				}
			}
		...
		public virtual void DrawSpriteMirrored(Graphics g, PASpriteInst theSpriteInst, PATransform? theTransform, Color theColor, bool additive, bool parentFrozen)
		{
			...
			for (int i = 0; i < paframe.mFrameObjectPosVector.Length; i++)
			{
				...
				if (color.mAlpha != 0)
				{
					if (paobjectPos.mIsSprite)
					{
						this.DrawSpriteMirrored(g, theSpriteInst.mChildren[paobjectPos.mObjectNum].mSpriteInst, new PATransform?(patransform2), new Color(color), paobjectPos.mIsAdditive || additive, flag);
					}
					else
					{
						PAImage paimage = this.mImageVector[(int)paobjectPos.mResNum];
						PATransform theSrcTransform = new PATransform(true);
						patransform2.TransformSrc(paimage.mTransform, ref theSrcTransform);
						g.SetColorizeImages(true);
						g.SetColor(color);
						if (additive || paobjectPos.mIsAdditive)
						{
						...
						if (this.mImgScale != 1f)
						{
							float m = theSrcTransform.mMatrix.m02;
							float m2 = theSrcTransform.mMatrix.m12;
							PATransform patransform4 = new PATransform(true);
							patransform4.mMatrix.m00 = 1f / this.mImgScale;
							patransform4.mMatrix.m11 = 1f / this.mImgScale;
							patransform4.TransformSrc(theSrcTransform, ref theSrcTransform);
							theSrcTransform.mMatrix.m02 = m;
							theSrcTransform.mMatrix.m12 = m2;
						}
						if (this.mDrawScale != 1f)
						{
							theSrcTransform.mMatrix.m02 = theSrcTransform.mMatrix.m02 + (float)this.mAnimRect.mWidth * (1f - this.mDrawScale);
						}
						if ((double)theSrcTransform.mMatrix.m00 == 1.0 && theSrcTransform.mMatrix.m01 == 0f && theSrcTransform.mMatrix.m10 == 0f && (double)theSrcTransform.mMatrix.m11 == 1.0)
						{
							float theX = (float)(this.mAnimRect.mWidth - theSrcRect.mWidth) / 2f - (theSrcTransform.mMatrix.m02 + (float)(theSrcRect.mWidth - this.mAnimRect.mWidth) / 2f);
							float m3 = theSrcTransform.mMatrix.m12;
							g.DrawImageF(image, theX, m3, theSrcRect);
						}
						else if (this.mVersion == 1 || (theSrcTransform.mMatrix.m00 == theSrcTransform.mMatrix.m11 && theSrcTransform.mMatrix.m01 == -theSrcTransform.mMatrix.m10 && Math.Abs((double)(theSrcTransform.mMatrix.m00 * theSrcTransform.mMatrix.m00 + theSrcTransform.mMatrix.m01 * theSrcTransform.mMatrix.m01) - 1.0) < 0.01))
						{
							float num = (float)Math.Atan2((double)theSrcTransform.mMatrix.m01, (double)theSrcTransform.mMatrix.m00);
							float num2 = -num;
							float num3 = theSrcTransform.mMatrix.m02 + (float)Math.Cos((double)num2) * (float)theSrcRect.mWidth / 2f - (float)Math.Sin((double)num2) * (float)theSrcRect.mHeight / 2f;
							float num4 = theSrcTransform.mMatrix.m12 + (float)Math.Sin((double)num2) * (float)theSrcRect.mWidth / 2f + (float)Math.Cos((double)num2) * (float)theSrcRect.mHeight / 2f;
						...
						else
						{
							...
							theMat.m02 = (float)theSrcRect.mWidth / 2f;
							theMat.m12 = (float)theSrcRect.mHeight / 2f;
							theSrcTransform.mMatrix.CopyFrom(theSrcTransform.mMatrix * theMat);
							theSrcTransform.mMatrix.m02 = (float)this.mAnimRect.mWidth - theSrcTransform.mMatrix.m02;
							theSrcTransform.mMatrix.m01 = -theSrcTransform.mMatrix.m01;
							theSrcTransform.mMatrix.m10 = -theSrcTransform.mMatrix.m10;
							g.DrawImageMatrix(image, theSrcTransform.mMatrix, theSrcRect);
						}
					}
				}
			}
		...
		public virtual bool Load_LoadPam(string theFileName)
		{
			...
			if (flag2)
			{
				this.mMainAnimDef.mMainSpriteDef = new PASpriteDef();
				if (!this.LoadSpriteDef(buffer, this.mMainAnimDef.mMainSpriteDef))
				{
					return false;
				}
			}
		...
		public virtual SharedImageRef Load_GetImageHook(string theFileDir, string theOrigName, string theRemappedName)
		{
			...
			for (int i = 0; i < this.mImageSearchPathVector.Count; i++)
			{
				...
				if (text.Length > 0 && text.get_Chars(text.Length - 1) != '\\' && text.get_Chars(text.Length - 1) != '/')
				{
					text += "\\";
				}
		...
		public virtual bool Load_GetImage(PAImage theImage, string theFileDir, string theOrigName, string theRemappedName)
		{
			...
			{
				theImage.mTransform.mMatrix.m20 = theImage.mTransform.mMatrix.m20 + -((float)sharedImageRef.mWidth - (float)theImage.mOrigWidth * this.mImgScale) / (float)(sharedImageRef.GetImage().mNumCols + 1);
				theImage.mTransform.mMatrix.m21 = theImage.mTransform.mMatrix.m21 + -((float)sharedImageRef.mHeight - (float)theImage.mOrigHeight * this.mImgScale) / (float)(sharedImageRef.GetImage().mNumRows + 1);
			}
		...
		{
			PAObjectPos[] cop_aNextObjectPos = new PAObjectPos[3];
			this.COP_aNextObjectPos = cop_aNextObjectPos;
			this.COP_anOfsTab = new int[]
			{
				default(int),
				1,
				2
		...
		public bool Play(int theFrameNum, bool resetAnim)
		{
			...
			if (theFrameNum >= this.mMainSpriteInst.mDef.mFrames.Length)
			{
				this.mAnimRunning = false;
				return false;
			}
		...
		public bool BlendTo(string theFrameLabel, int theBlendTicks, int theAnimStartDelay)
		{
			...
			if (!this.mTransDirty)
			{
				this.UpdateTransforms(this.mMainSpriteInst, default(PATransform?), new Color(this.mColor), false);
			...
			for (int i = 0; i < paframe.mFrameObjectPosVector.Length; i++)
			{
				...
				if (paobjectInst.mName != null && paobjectInst.mName.get_Chars(0) != '\0')
				{
					Color color;
					if (paobjectPos.mIsSprite)
					{
					...
					{
						this.CalcObjectPos(this.mMainSpriteInst, i, false, ref patransform, out color);
					}
			...
			for (int j = 0; j < this.mMainSpriteInst.mDef.mObjectDefVector.Count; j++)
			{
				...
				if (paobjectInst2.mName != null && paobjectInst2.mName.get_Chars(0) != '\0')
				{
					BlendSrcData blendSrcData2 = null;
					if (dictionary.TryGetValue(paobjectInst2.mName, ref blendSrcData2))
					{
		...
		public override void Draw(Graphics g)
		{
			...
			if (this.mTransDirty)
			{
				this.UpdateTransforms(this.mMainSpriteInst, default(PATransform?), this.mColor, false);
				this.mTransDirty = false;
			}
			if (this.mMirror)
			{
				this.DrawSpriteMirrored(g, this.mMainSpriteInst, default(PATransform?), this.mColor, this.mAdditive, false);
				return;
			}
			this.DrawSprite(g, this.mMainSpriteInst, default(PATransform?), this.mColor, this.mAdditive, false);
		}
		...
		public static ulong PAM_MAGIC = (ulong)(-1158669996);
		...
		private PAObjectPos[] COP_aNextObjectPos;

		private int[] COP_anOfsTab;
	}
}
