...
namespace SexyFramework.Sound
{
	public class XSoundManager : SoundManager
	{
		...
		public override int LoadSound(string theFilename)
		{
			for (int i = 0; i < XSoundManager.MAX_SOURCE_SOUNDS; i++)
			{
				if (this.m_sounds[i] != null && this.m_sounds[i].mFileName == theFilename)
				{
					return i;
				}
			}
			int j = XSoundManager.MAX_SOURCE_SOUNDS - 1;
			while (j >= 0)
			{
				if (this.m_sounds[j] == null)
				{
					if (this.LoadSound((uint)j, theFilename))
					{
						return j;
					}
					break;
		...
		public override void Update()
		{
			for (int i = this.mInstances.Count - 1; i >= 0; i--)
			{
				...
				else if (this.mInstances[i].IsReleased())
				{
