	...
	internal class ResourceManager : IDisposable
	{
		...
		private Texture2D LoadTextureFromStream(string filename, bool premultiply, ImageRes.TextureFormat format, SurfaceFormat lowMemorySurfaceFormat)
		{
				...
				if (texture2D.Width * texture2D.Height < 4194304)
				{
					...
					try
					{
						...
						lock (SexyAppBase.SplashScreenDrawLock)
						{
							...
							{
								renderTarget2D = new RenderTarget2D(graphicsDevice, texture2D.Width, texture2D.Height, false, lowMemorySurfaceFormat, 0, 0, 0);
							}
							else
							{
								renderTarget2D = new RenderTarget2D(graphicsDevice, texture2D.Width, texture2D.Height, false, 0, 0, 0, 0);
							}
							graphicsDevice.SetRenderTarget(renderTarget2D);
							graphicsDevice.Clear(Color.Black);
							ResourceManager.imageLoadSpritebatch.Begin(1, this.blendColorLoadState);
							ResourceManager.imageLoadSpritebatch.Draw(texture2D, texture2D.Bounds, Color.White);
							ResourceManager.imageLoadSpritebatch.End();
							ResourceManager.imageLoadSpritebatch.Begin(1, this.imageLoadBlendAlpha);
							ResourceManager.imageLoadSpritebatch.Draw(texture2D, texture2D.Bounds, Color.White);
							ResourceManager.imageLoadSpritebatch.End();
			...
			if (!flag)
			{
				...
				if (Main.DO_LOW_MEMORY_OPTIONS)
				{
					Texture2D texture2D2 = new Texture2D(graphicsDevice, texture2D.Width, texture2D.Height, false, 3);
