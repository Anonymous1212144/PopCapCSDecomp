	...
	internal class ZombiePileWidget : Widget, ButtonListener
	{
		...
		public override void Draw(Graphics g)
		{
			...
			for (int j = 0; j < this.mPileHeight; j++)
			{
				...
				num2 -= num4 - ZombiePileWidget.mZombieSpace;
			}
			...
			for (int i = 0; i < this.gPileObjects.Length; i++)
			{
				...
				if (zombiePileObject.mHeight <= this.mPileHeight)
				{
					...
					case ZombiePileObjectType.OBJECT_BLACKHOLE:
					{
						...
						Vector3 vector;
						vector..ctor(0f, (float)num, 0f);
						Vector3 vector2;
						vector2..ctor(-60f, 80f, 0f);
						world = Matrix.CreateTranslation(vector) * Matrix.CreateScale(4f) * Matrix.CreateRotationZ(zombiePileObject.mCounter) * Matrix.CreateRotationX(-0.85f) * Matrix.CreateTranslation(-vector) * Matrix.CreateTranslation(vector2);
			...
			for (int i = 0; i < this.gPileObjects.Length; i++)
			{
				...
				if (zombiePileObject.mHeight <= this.mPileHeight)
				{
					...
					case ZombiePileObjectType.OBJECT_BALLOON:
						...
						if ((double)zombiePileObject.mCounter > 6.2831853071795862)
						{
							zombiePileObject.mCounter -= 6.28318548f;
						}
		...
		private short[] blackHoleIndices = new short[] { 0, 1, 2, 1, 3, 2 };
