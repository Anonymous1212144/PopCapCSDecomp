...
namespace Sexy
{
	public class ScrollWidget : InterfaceControl, ProxyWidgetListener
	{
		...
		public override void TouchBegan(_Touch touch)
		{
			if (this.mClient != null)
			{
				...
				touch.location.mX = touch.location.mX - this.mScrollOffset.mX;
				touch.location.mY = touch.location.mY - this.mScrollOffset.mY;
			...
			if (this.mVisible && !this.mDisabled)
			{
				...
				else
				{
					...
					if (this.mScrollOffset.x < num)
					{
						...
						{
							this.mScrollOffset.x = this.mScrollOffset.x + 0.1f * (num - this.mScrollOffset.x);
						}
						else
						{
					...
					else if (this.mScrollOffset.x > num3)
					{
						if (this.mBounceEnabled || this.mSeekScrollTarget)
						{
							this.mScrollOffset.x = this.mScrollOffset.x + 0.1f * (num3 - this.mScrollOffset.x);
						}
						else
						{
					...
					if (this.mScrollOffset.y < num2)
					{
						if (this.mBounceEnabled || this.mSeekScrollTarget)
						{
							this.mScrollOffset.y = this.mScrollOffset.y + 0.1f * (num2 - this.mScrollOffset.y);
						}
						else
						{
					...
					else if (this.mScrollOffset.y > num4)
					{
						if (this.mBounceEnabled || this.mSeekScrollTarget)
						{
							this.mScrollOffset.y = this.mScrollOffset.y + 0.1f * (num4 - this.mScrollOffset.y);
						}
						else
						{
		...
		public override void Draw(Graphics g)
		{
			...
			if (this.mFillBackground)
			{
				...
				g.SetColor(Color.Multiply(this.GetColor(0), this.mOpacity));
		...
		{
			Color color;
			color..ctor(255, 255, 255, (int)(255f * this.mIndicatorsOpacity * this.mOpacity));
			if (color.A != 0)
			{
			...
			if (this.mDrawOverlays)
			{
				...
				foreach (ScrollWidget.Overlay overlay in this.mOverlays)
				{
					g.DrawImage(overlay.image, (int)overlay.offset.mX, (int)overlay.offset.mY, this.mWidth, overlay.image.GetHeight());
				}
			}
		}
			...
			if ((this.mScrollPractical & ScrollWidget.ScrollMode.SCROLL_HORIZONTAL) != ScrollWidget.ScrollMode.SCROLL_DISABLED)
			{
				...
				else
				{
					...
					if (num2 > 0.0)
					{
						...
						double num4 = (double)Math.Min(1f, (float)num2 / 0f);
		...
		public void CacheDerivedValues()
		{
			if (this.mClient != null)
			{
