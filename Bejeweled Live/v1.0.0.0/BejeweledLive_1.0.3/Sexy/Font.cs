...
namespace Sexy
{
	public class Font
	{
		...
		private Vector2 scale
		{
			get
			{
				return new Vector2(this.mScaleX * Constants.mConstants.Font_Scale, this.mScaleY * Constants.mConstants.Font_Scale);
			}
		}
		...
		public int mHeight
		{
			get
			{
				return (int)((float)this.height * this.mScaleY * Constants.mConstants.Font_Scale);
			}
			set
			{
				this.height = value;
		...
		public Font.CachedStringInfo GetWordWrappedSubStrings(string theLine, TRect theRect)
		{
			...
			if (!dictionary.TryGetValue(theLine, ref cachedStringInfo))
			{
				...
				if (num4 + this.StringWidth(theLine.Substring(num3, theLine.Length - num3)) > theRect.mWidth)
				{
					list.Add(new StringBuilder());
				}
		...
		{
			return (int)((float)this.mAscent * this.mScaleY);
		}
		...
		private int ComputeStringWidthForCache(string theString)
		{
			...
			{
				float num2 = this.mOffsets[i].X * this.mScaleX * Constants.mConstants.Font_Scale;
		...
		public int StringWidth(StringBuilder theString)
		{
			...
			if (this.StringWidthCachingEnabled)
			{
				Dictionary<char, int> newCharDictionary;
				if (!this.cachedStringBuilderWidths.TryGetValue(this.mScaleX, ref newCharDictionary))
				{
					newCharDictionary = Font.GetNewCharDictionary();
					this.cachedStringBuilderWidths.Add(this.mScaleX, newCharDictionary);
				}
				if (!newCharDictionary.TryGetValue(theString.get_Chars(0), ref num))
				{
					num = this.ComputeStringWidthForCache(theString);
					if (!newCharDictionary.ContainsKey(theString.get_Chars(0)))
					{
						newCharDictionary.Add(theString.get_Chars(0), num);
					}
				}
			}
			else
		...
		private int ComputeStringWidthForCache(StringBuilder theString)
		{
			...
			for (int i = 0; i < this.mFonts.Count; i++)
			{
				...
				if (theString.Length <= 1)
				{
					...
					{
						num2 += (int)(this.mFonts[i].MeasureString(theString).X * this.mScaleX * Constants.mConstants.Font_Scale);
					}
				}
				else
				{
		...
		{
			Vector2 vector;
			vector..ctor((float)theX, (float)theY);
			Vector2 vector2 = vector + this.mOffsets[layer] * this.mScaleX * Constants.mConstants.Font_Scale;
			if (!string.IsNullOrEmpty(theString) && this.mCharOffsets.ContainsKey(theString.get_Chars(0)))
			{
				vector2.X -= this.mCharOffsets[theString.get_Chars(0)].X * this.mScaleX * Constants.mConstants.Font_Scale;
			}
			for (int i = 0; i < theString.Length; i++)
			{
				if (char.IsWhiteSpace(theString.get_Chars(i)))
				{
					vector2.X += (float)this.StringWidth(this.SpaceChar);
				}
				else
				{
					...
					{
						vector3 = vector2 + this.mCharOffsets[theString.get_Chars(i)] * this.mScaleX * Constants.mConstants.Font_Scale;
					}
					else
					{
						vector3 = vector2;
					}
					Graphics.spriteBatch.DrawString(this.mFonts[layer], this.drawStringBuilder, vector3, theColor, 0f, Vector2.Zero, this.scale, 0, 1f - (float)layer / (float)this.mFonts.Count);
					vector2.X += (float)(this.StringWidth(this.drawStringBuilder) - this.characterOffsetMagic);
				}
			}
		}
		...
		public void DrawString(Graphics g, int theX, int theY, StringBuilder theString, Color theColor)
		{
			...
			for (int i = 0; i < this.mFonts.Count; i++)
			{
				Vector2 vector2 = vector + this.mOffsets[i] * this.mScaleX * Constants.mConstants.Font_Scale;
				for (int j = 0; j < theString.Length; j++)
				{
					if (char.IsWhiteSpace(theString.get_Chars(j)))
					{
					...
					else
					{
						...
						{
							vector3 = vector2 + this.mCharOffsets[theString.get_Chars(j)] * this.mScaleX * Constants.mConstants.Font_Scale;
						}
						else
						{
							vector3 = vector2;
						}
						Graphics.spriteBatch.DrawString(this.mFonts[i], this.drawStringBuilder, vector3, theColor, 0f, Vector2.Zero, this.scale, 0, 1f - (float)i / (float)this.mFonts.Count);
						vector2.X += (float)(this.StringWidth(this.drawStringBuilder) - this.characterOffsetMagic);
					}
				}
			}
		...
		public class CachedStringInfo
		{
			...
			public CachedStringInfo(string[] substrings, Font fontUsed)
			{
				...
				for (int i = 0; i < this.Strings.Length; i++)
				{
					...
					int num = i;
					stringDimensions[num].Y = stringDimensions[num].Y + (float)fontUsed.GetHeight() * 0.2f;
				}
			}
